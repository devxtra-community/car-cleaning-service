...
export default function Collections() {
    ...
    const loadCollections = async () => {
        try {
            const res = await api.get('/api/tasks/collections/supervisor');
    ...
    const totalCash = collections.reduce((s, c) => s + Number(c.cash), 0);
    const totalUPI = collections.reduce((s, c) => s + Number(c.upi), 0);
    const totalAll = collections.reduce((s, c) => s + Number(c.total), 0);
                ...
                >
                    ...
                    <View style={{ flexDirection: 'row', gap: 10, marginBottom: 20 }}>
                        {[
                            ...
                            { label: 'UPI', value: totalUPI, color: '#0EA5E9', bg: '#EFF6FF', icon: <Smartphone size={16} color="#0EA5E9" /> },
