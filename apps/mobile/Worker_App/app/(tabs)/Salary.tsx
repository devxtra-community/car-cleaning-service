...
export default function SalaryScreen() {
    ...
    const fetchTimeline = useCallback(async () => {
        try {
            ...
            let decoded: any;
            try { decoded = jwtDecode(token); } catch { setLoading(false); return; }
            ...
            if (res.data?.success) {
                ...
                setTimeline(data);
                const now = new Date();
                const cur = data.find(m => m.year === now.getFullYear() && m.month === now.getMonth() + 1);
                selectEntry(cur ?? data[0] ?? null);
                setYear(now.getFullYear());
            }
        } catch (e) { console.error('Failed to fetch salary timeline', e); }
    ...
    const onRefresh = async () => { setRefreshing(true); await fetchTimeline(); setRefreshing(false); };
        ...
        <View style={{ flex: 1, backgroundColor: '#F8FAFC' }}>
            ...
            >
                ...
                >
                    ...
                    <Text style={{ fontSize: 14, color: 'rgba(255,255,255,0.85)', fontWeight: '600' }}>{t('common.back')}</Text>
                </TouchableOpacity>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}
                    style={{ marginTop: 16 }} contentContainerStyle={{ gap: 8 }}>
                    {availableYears.map(y => (
                        ...
                        >
                            <Text style={[styles.yearTabText, year === y && styles.yearTabTextActive]}>
                                {y}
                            </Text>
                        </TouchableOpacity>
                    ))}
                            ...
                            <Animated.View style={{ opacity: fadeAnim, marginHorizontal: 16, marginTop: 8 }}>
                                ...
                                <View style={styles.cardHeader}>
                                    <View>
                                        <Text style={styles.cardMonth}>
                                            {t(`common.months.${MONTH_NAMES[selected.month - 1].toLowerCase()}`)} {selected.year}
                                        </Text>
                                ...
                                >
                                    ...
                                    <View style={styles.heroSub}>
                                        <Text style={styles.heroSubText}>{t('salary.base')} {fmt(selected.base_salary)}</Text>
                                ...
                                <View style={styles.breakdownCard}>
                                    <Row label={t('salary.baseSalary')} value={fmt(selected.base_salary)} valueColor="#1E293B" />
                                    <Div />
                                    <Row label={`✦ ${t('salary.incentivesEarned')}`} value={`+${fmt(selected.incentives)}`} valueColor="#059669" />
                                    <Div />
                                    <Row label={`✦ ${t('salary.penaltiesDeducted')}`} value={`-${fmt(selected.penalties)}`} valueColor="#DC2626" />
                                    <Div />
                                    <Row label={t('salary.netSalary')} value={fmt(selected.final_salary)} valueColor="#0C4A6E" bold />
                                </View>
...
const Row = ({ label, value, valueColor, bold }: { label: string; value: string; valueColor: string; bold?: boolean }) => (
    <View style={styles.row}>
        ...
        <Text style={[styles.rowValue, { color: valueColor }, bold && { fontSize: 16, fontWeight: '800' }]}>{value}</Text>
    </View>
);
const Div = () => <View style={{ height: 1, backgroundColor: '#F1F5F9', marginVertical: 2 }} />;

const styles = StyleSheet.create({
    headerSub: { fontSize: 11, color: 'rgba(255,255,255,0.65)', letterSpacing: 2, textTransform: 'uppercase' },
    headerTitle: { fontSize: 28, fontWeight: '800', color: '#fff', marginTop: 2 },
    yearTab: { paddingHorizontal: 16, paddingVertical: 7, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.15)' },
    ...
    stripWrapper: { backgroundColor: '#fff', marginTop: 0, borderBottomWidth: 1, borderBottomColor: '#F1F5F9', shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 4, elevation: 2 },
    chip: { alignItems: 'center', justifyContent: 'center', paddingHorizontal: 14, paddingVertical: 10, borderRadius: 14, backgroundColor: '#F8FAFC', minWidth: 56, shadowColor: '#000', shadowOpacity: 0, shadowRadius: 0, elevation: 0 },
    ...
    chipNow: { fontSize: 8, color: '#0284C7', fontWeight: '700', marginTop: 2, textTransform: 'uppercase', letterSpacing: 0.5 },

    cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10, marginTop: 16 },
    cardMonth: { fontSize: 22, fontWeight: '800', color: '#0F172A' },
    currentBadge: { marginTop: 3, backgroundColor: '#DBEAFE', alignSelf: 'flex-start', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8 },
    ...
    heroLabel: { fontSize: 11, color: 'rgba(255,255,255,0.65)', textTransform: 'uppercase', letterSpacing: 1.5, fontWeight: '600' },
    ...
    breakdownCard: { backgroundColor: '#fff', borderRadius: 18, padding: 18, marginBottom: 12, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 8, elevation: 2 },
    row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10 },
    rowLabel: { fontSize: 13, color: '#64748B', fontWeight: '500' },
    rowValue: { fontSize: 14, fontWeight: '700' },

    notice: { backgroundColor: '#FEF9EC', borderRadius: 14, padding: 14, borderWidth: 1, borderColor: '#FCD34D' },
