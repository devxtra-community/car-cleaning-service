          ...
          filteredWorkers.map((worker) => (
            ...
            >
              <View className="flex-row items-center gap-4 flex-1">
                <View
                  className={`w-12 h-12 rounded-2xl items-center justify-center ${worker.attendance_status === 'present' ? 'bg-green-50' : 'bg-red-50'}`}
                >
                  <User
                    size={24}
                    color={worker.attendance_status === 'present' ? '#22C55E' : '#FB7185'}
                  />
                </View>
                    ...
                    {worker.attendance_status === 'present' ? (
                      <View className="flex-row items-center gap-1">
                        ...
                        <Text className="text-xs text-gray-400">
                          Checked in{' '}
                          {new Date(worker.check_in_time!).toLocaleTimeString([], {
                            hour: '2-digit',
                            minute: '2-digit',
                          })}
                        </Text>
