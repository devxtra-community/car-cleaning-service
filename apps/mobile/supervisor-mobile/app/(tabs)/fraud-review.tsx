...
import { ShieldAlert, CheckCircle, AlertTriangle, Clock } from 'lucide-react-native';
import api from '../../src/api/api';
import { useLanguage } from '@/contexts/LanguageContext';

interface FraudCase {
  id: string;
  type: 'missing_photo' | 'too_fast' | 'duplicate_vehicle';
...
export default function FraudReview() {
  ...
  const loadCases = async () => {
    try {
      const res = await api.get('/fraud/pending');
  ...
  const handleStatusUpdate = async (id: string, newStatus: 'resolved' | 'escalated') => {
    try {
      setUpdating(id);
      await api.patch(`/fraud/${id}/status`, { status: newStatus });
  ...
  const getFraudIcon = (type: string) => {
    switch (type) {
      ...
      default:
        return <AlertTriangle size={20} color="#94A3B8" />;
    }
  };
    ...
    <View style={{ flex: 1, backgroundColor: '#F8FAFC', paddingTop: insets.top }}>
      ...
      >
        <Text style={{ fontSize: 22, fontWeight: '800', color: '#1E293B' }}>
          {t('supervisor.fraud', { defaultValue: 'Fraud Review' })}
        </Text>
        <Text style={{ fontSize: 12, color: '#94A3B8', marginTop: 4 }}>
          {t('supervisor.actionRequired', {
            defaultValue: 'Action required on flagged tasks across your building',
          })}
        </Text>
      </View>
          ...
          {cases.length === 0 ? (
            <View style={{ alignItems: 'center', marginTop: 60 }}>
              ...
              <Text style={{ fontSize: 16, fontWeight: '700', color: '#10B981', marginTop: 16 }}>
                {t('supervisor.allClearFraud', {
                  defaultValue: 'All clear! No fraud cases pending.',
                })}
              </Text>
            </View>
          ) : (
            cases.map((c) => (
              ...
              >
                ...
                >
                  <Text
                    style={{ fontSize: 13, fontWeight: '600', color: '#1E293B', marginBottom: 4 }}
                  >
                    {c.cleaner_name}
                  </Text>
                ...
                <View style={{ flexDirection: 'row', gap: 10 }}>
                  ...
                  >
                    {updating === c.id ? (
                      ...
                      <Text style={{ color: '#10B981', fontWeight: '700', fontSize: 12 }}>
                        {t('supervisor.resolved', { defaultValue: 'Mark Resolved' })}
                      </Text>
                    )}
                  </Pressable>
                  ...
                  >
                    <Text style={{ color: '#EF4444', fontWeight: '700', fontSize: 12 }}>
                      {t('supervisor.escalate', { defaultValue: 'Escalate' })}
                    </Text>
                  </Pressable>
                </View>
              </View>
