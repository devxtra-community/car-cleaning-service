    ...
    <View style={{ flex: 1, backgroundColor: '#F8FAFC', paddingTop: insets.top }}>
      ...
      >
        <Text style={{ fontSize: 22, fontWeight: '800', color: '#1E293B' }}>
          {t('supervisor.photoVerification', { defaultValue: 'Photo Verification' })}
        </Text>
        <Text style={{ fontSize: 12, color: '#94A3B8', marginTop: 4 }}>
          {t('supervisor.reviewPhotos', {
            defaultValue: 'Review before/after photos and approve or flag jobs',
          })}
        </Text>
      </View>
          ...
          {jobs.length === 0 ? (
            <View style={{ alignItems: 'center', marginTop: 60 }}>
              ...
              <Text style={{ fontSize: 16, fontWeight: '700', color: '#94A3B8', marginTop: 16 }}>
                {t('supervisor.noJobsToVerify', { defaultValue: 'No jobs to verify' })}
              </Text>
            </View>
          ) : (
            jobs.map((job) => (
              ...
              >
                ...
                >
                  ...
                  {job.verification_status && (
                    ...
                    >
                      ...
                      >
                        {job.verification_status === 'approved'
                          ? t('supervisor.approve', { defaultValue: 'Approve' }).toUpperCase()
                          : t('supervisor.flag', { defaultValue: 'Flag' }).toUpperCase()}
                      </Text>
                    </View>
                  )}
                </View>
                ...
                <View style={{ flexDirection: 'row', gap: 8, marginBottom: 14 }}>
                  {[
                    {
                      label: t('supervisor.before', { defaultValue: 'Before' }),
                      url: job.car_image_url ?? job.before_wash_image_url,
                    },
                    {
                      label: t('supervisor.after', { defaultValue: 'After' }),
                      url: job.after_wash_image_url,
                    },
                  ].map(({ label, url }) => (
                      ...
                      {url ? (
                        ...
                        >
                          <Text style={{ fontSize: 11, color: '#CBD5E1' }}>
                            {t('supervisor.noPhoto', { defaultValue: 'No Photo' })}
                          </Text>
                        </View>
                      )}
                    </View>
                  ...
                  <View style={{ flexDirection: 'row', gap: 10 }}>
                      ...
                      {submitting === job.id ? (
                        ...
                        <>
                          ...
                          <Text style={{ color: '#fff', fontWeight: '700', fontSize: 12 }}>
                            {t('supervisor.approve', { defaultValue: 'Approve' })}
                          </Text>
                        </>
                      )}
                    </Pressable>
                    ...
                    >
                      ...
                      <Text style={{ color: '#EF4444', fontWeight: '700', fontSize: 12 }}>
                        {t('supervisor.flag', { defaultValue: 'Flag' })}
                      </Text>
                    </Pressable>
                  </View>
                )}
