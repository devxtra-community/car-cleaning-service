...
export default function OperationalReports() {
    ...
    const fetchReportData = async (selectedPeriod: ReportPeriod) => {
        setLoading(true);
        try {
            const res = await api.get(`/analytics/${selectedPeriod}`);
    ...
    const getPeriodLabel = (row: AnalyticsRow) => {
        ...
        if (period === 'weekly') return row.week_start ? `Week of ${new Date(row.week_start).toLocaleDateString()}` : 'N/A';
        if (period === 'monthly') return row.month ? new Date(row.month).toLocaleDateString(undefined, { month: 'long', year: 'numeric' }) : 'N/A';
        return 'Unknown';
    };
    ...
    const handleExportExcel = () => {
        ...
        const worksheetData = [
            headers,
            ...data.map(row => rawKeys.map(k => row[k]))
        ];

        const workbook = XLSX.utils.book_new();
        const worksheet = XLSX.utils.aoa_to_sheet(worksheetData);
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Operational Report');
        XLSX.writeFile(workbook, `${period}_operational_report_${new Date().toISOString().split('T')[0]}.xlsx`);
    };
    ...
    const handleExportCSV = () => {
        ...
        link.href = url;
        link.setAttribute('download', `${period}_operational_report_${new Date().toISOString().split('T')[0]}.csv`);
                    ...
                    <table className="w-full text-left text-sm text-slate-600">
                        <thead className="bg-slate-50 border-b border-slate-200 text-slate-500">
                            <tr>
                                ...
                                <th className="px-6 py-3 font-medium text-right">Active Cleaners</th>
                                <th className="px-6 py-3 font-medium text-right">Wash Time (avg)</th>
                                <th className="px-6 py-3 font-medium text-right">Incentives ($)</th>
                                <th className="px-6 py-3 font-medium text-right">Penalties ($)</th>
                            </tr>
                        </thead>
                            ...
                            {loading ? (
                                ...
                                data.map((row, i) => (
                                    <tr key={i} className="hover:bg-slate-50 transition">
                                        ...
                                        <td className="px-6 py-4 text-right">{row.total_cleaners || 0}</td>
                                        <td className="px-6 py-4 text-right">
                                            {row.average_wash_time ? `${Number(row.average_wash_time).toFixed(1)} mins` : '-'}
                                        </td>
                                        <td className="px-6 py-4 text-emerald-600 text-right font-medium">
                                            {row.total_incentives ? `+${Number(row.total_incentives).toFixed(2)}` : '-'}
                                        </td>
                                        <td className="px-6 py-4 text-red-600 text-right font-medium">
                                            {row.total_penalties ? `-${Number(row.total_penalties).toFixed(2)}` : '-'}
                                        </td>
                                    </tr>
                                ))
                            )}
...
function PeakActivitySection({ period }: { period: string }) {
    ...
    useEffect(() => {
        (async () => {
            setLoading(true);
            try {
                const res = await api.get(`/analytics/peak-activity?period=${period}`);
                setData(res.data?.data || []);
            } catch (err) {
    ...
    return (
        <div className="mt-8 bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden p-6">
            ...
            <p className="text-sm text-slate-600 mb-6">Hourly distribution of completed tasks over the selected period</p>
