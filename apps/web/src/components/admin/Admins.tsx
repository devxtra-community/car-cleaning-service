...
const EditAdminModal: React.FC<{
  admin: AdminListItem;
  onClose: () => void;
  onSaved: (updated: AdminListItem) => void;
  ...
  const [form, setForm] = useState<UserUpdatePayload>({
    ...
    document_id: admin.document_id ?? '',
    base_salary: admin.base_salary ?? undefined,
    joining_date: admin.joining_date?.slice(0, 10) ?? '',
    password: '',
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
          <div>
            <h2 className="text-base font-extrabold text-slate-900">Edit Admin</h2>
            <p className="text-xs text-slate-400 mt-0.5">{admin.full_name}</p>
          </div>
          <button
            onClick={onClose}
...
const Admins: React.FC = () => {
  ...
  }, [admins, search, statusF, sortKey, sortDir]);
  ...
  const handleToggle = async (a: AdminListItem) => {
    const next = !a.is_active;
    setToggling(a.id);
    try {
      await toggleAdminStatus(a.id, next);
      setAdmins((p) =>
        p.map((x) => (x.id === a.id ? { ...x, is_active: next, token_version: next ? 0 : -1 } : x))
      );
      showToast(`Admin ${next ? 'activated' : 'deactivated'}`, 'success');
    } catch {
  ...
  const handleSaved = (updated: AdminListItem) => {
    setAdmins((p) =>
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-100 sticky top-0 z-20">
        <div className="mx-auto px-6 py-4 flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <h1 className="text-lg font-extrabold text-slate-900 tracking-tight">Admins</h1>
          </div>
          <Link
      ...
      <div className="mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {(
            [
              { label: 'Total', value: admins.length, sub: 'registered', color: 'text-slate-900' },
                      ...
                      <td colSpan={7}>
                        <div className="flex flex-col items-center justify-center py-24 gap-3">
                          ...
                          <p className="text-sm text-slate-400">
                            {search ? `No results for "${search}"` : 'No admins yet'}
                          </p>
                        </div>
                      </td>
                    </tr>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3.5 text-sm text-slate-600">
                            {a.nationality ?? <span className="italic text-slate-300">—</span>}
                          </td>
                          {/* salary */}
                          <td className="px-4 py-3.5 text-right font-semibold text-slate-800 text-sm whitespace-nowrap">
                            {a.base_salary != null
                              ? `${Number(a.base_salary).toLocaleString('en-AE', { minimumFractionDigits: 2 })} AED`
                              : '—'}
                          </td>
                          {/* joined */}
