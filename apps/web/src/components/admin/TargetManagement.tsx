        ...
        <div className="p-6 bg-slate-50 min-h-screen">
            <div className="flex items-center justify-between mb-6 border-b border-slate-200 pb-4">
                <div>
                    ...
                    <p className="text-sm text-slate-600 mt-1">Configure daily task targets and incentives</p>
                </div>
                <button
                            ...
                            targets.map((t) => (
                                <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        {t.cleaner_name ? (
                                            <span className="bg-blue-50 text-blue-700 px-2 py-1 rounded text-[11px] font-bold">Cleaner: {t.cleaner_name}</span>
                                        ) : t.building_name ? (
                                            <span className="bg-emerald-50 text-emerald-700 px-2 py-1 rounded text-[11px] font-bold">Building: {t.building_name}</span>
                                        ) : t.floor_name ? (
                                            <span className="bg-amber-50 text-amber-700 px-2 py-1 rounded text-[11px] font-bold">Floor: {t.floor_name}</span>
                                        ) : (
                                            <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded text-[11px] font-bold">Global</span>
                                        )}
                                    </td>
                                    <td className="px-6 py-4 text-slate-700 font-semibold">{t.target_tasks} Tasks</td>
                                    <td className="px-6 py-4 text-emerald-600 font-semibold">₹{t.incentive_amount}</td>
                                    <td className="px-6 py-4">
                                        <span
                                            className={`inline-flex px-2 py-1 rounded text-xs font-semibold ${t.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'
                                                }`}
                                        >
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center justify-center gap-3">
                                            <button onClick={() => openEditModal(t)} className="text-blue-600 hover:underline">
                                                Edit
                                            </button>
                    ...
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
                        ...
                        <form onSubmit={handleSave} className="p-6">
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">Target Name / Reason</label>
                                    <input
                                        type="text"
                                        required
                                        value={reason}
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">Daily Task Target</label>
                                    <input
                                        type="number"
                                        required
                                        min="1"
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">Incentive Amount (₹)</label>
                                    <input
                                        type="number"
                                        required
                                        min="1"
                                ...
                                <div className="pt-2 border-t border-slate-100 mt-2">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Optional Scope</p>
                                    <div className="space-y-3">
                                        <div>
                                            <label className="block text-[11px] font-medium text-slate-500 mb-1">Specific Cleaner</label>
                                            <select
                                                value={selectedCleaner}
                                                onChange={e => {
                                                    setSelectedCleaner(e.target.value);
                                                    if (e.target.value) { setSelectedBuilding(''); setSelectedFloor(''); }
                                                }}
                                        ...
                                        <div>
                                            <label className="block text-[11px] font-medium text-slate-500 mb-1">Specific Building</label>
                                            <select
                                                value={selectedBuilding}
                                                onChange={e => {
                                                    setSelectedBuilding(e.target.value);
                                                    if (e.target.value) { setSelectedCleaner(''); setSelectedFloor(''); }
                                                }}
                                                className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-blue-500"
                                            >
                                        ...
                                        <div>
                                            <label className="block text-[11px] font-medium text-slate-500 mb-1">Specific Floor</label>
                                            <select
                                                value={selectedFloor}
                                                onChange={e => {
                                                    setSelectedFloor(e.target.value);
                                                    if (e.target.value) { setSelectedCleaner(''); setSelectedBuilding(''); }
                                                }}
                                                className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-blue-500"
                                            >
                                                <option value="">All Floors</option>
                                                {floors.map(f => <option key={f.id} value={f.id}>{f.floor_name} ({f.building_name || 'Floor'})</option>)}
                                            </select>
                                        </div>
                                    </div>
                                </div>
