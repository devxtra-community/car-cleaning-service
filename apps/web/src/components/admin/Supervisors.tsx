    ...
    <>
      <div className="p-6 min-h-screen">
        ...
        <div className="grid grid-cols-3 gap-6">
          {supervisors.map((sup) => (
            <Link key={sup.id} to={`/admin/supervisors/${sup.id}/cleaners`}>
              <div className="bg-white rounded-xl p-4 flex items-center gap-4 shadow-sm hover:shadow-md transition cursor-pointer">
                <img src={profileImg} className="w-12 h-12 rounded-full" />
                <div>
                  <p className="font-medium">{sup.full_name}</p>
                  <p className="text-xs text-gray-500">{sup.email || 'Unknown'}</p>
                </div>
              </div>
            </Link>
          ))}
