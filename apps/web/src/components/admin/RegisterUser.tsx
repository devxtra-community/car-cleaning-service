...
const RegisterUser = () => {
  ...
  const handleSupervisorChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    try {
      // @ts-ignore - supervisor_id is returned by the API but interface has id currently
      const selectedSupervisor = supervisors.find((s) => (s.id || s.supervisor_id) === supervisorId);
        ...
        >
                  ...
                  <div className="md:col-span-2">
                    ...
                    >
                      ...
                      {supervisors.map((s: any) => (
                        <option key={s.id || s.supervisor_id} value={s.id || s.supervisor_id}>
                          {s.full_name}
                        </option>
                      ))}
                    </select>
          ...
          <div className="flex gap-3 pt-6 border-t border-slate-200">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`flex-1 px-6 py-3 rounded-lg text-white font-semibold transition flex items-center justify-center gap-2 ${loading
