...
export default function RoleBasedSalary() {
  ...
  const fetchSalaries = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/salary/role-summary${params}`);
  ...
  return (
    <div className="p-6 min-h-screen">
      ...
      <div className="flex gap-1 bg-white border border-slate-200 rounded-lg p-1 shadow-sm w-fit mb-6">
        {(['cleaner', 'supervisor', 'all'] as RoleTab[]).map((tab) => (
          <button
            ...
            className={`px-5 py-1.5 text-sm font-medium rounded-md capitalize transition ${
              roleTab === tab
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                filtered.map((row) => (
                  <tr
                    key={`${row.user_id}-${row.salary_id || 'no-sal'}`}
            ...
            {filtered.length > 0 && (
              <tfoot className="bg-slate-50 border-t border-slate-200 font-semibold text-sm">
                <tr>
                  <td colSpan={4} className="px-5 py-3 text-slate-700">
      ...
      {toast && (
        <div
          className={`fixed bottom-5 right-5 px-4 py-3 rounded-lg shadow-lg text-white text-sm z-50 ${
