...
const SalarySummary: React.FC = () => {
  ...
  const totalCollections = rows.reduce((s, r) => s + Number(r.total_collections || 0), 0);
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Salary Summary</h1>
          <p className="text-sm text-gray-600 mt-1">
            Performance overview - Daily, Weekly & Monthly
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Stat title="TOTAL TASKS" value={totalTasks.toString()} color="blue" />
        <Stat
          title="INCENTIVES"
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-200">
            {loading ? (
              ...
              rows.map((row) => {
                const net =
                  Number(row.total_collections) +
                  Number(row.total_incentives) -
                  Number(row.total_penalties);
                ...
                return (
                  <tr key={row.cleaner_id} className="hover:bg-gray-50">
                    ...
                    <td className="p-4 text-right text-purple-600">₹{row.total_collections}</td>
                    <td className="p-4 text-right font-semibold text-blue-700">₹{net}</td>
                  </tr>
                );
              })
            )}
