...
import { FraudType } from './fraud_types';

export class FraudService {
    /**
     * Runs fraud detection rules against a completed task and records any violations.
     */
    static async detectAndRecord(client: PoolClient, task: any): Promise<void> {
        const rules: FraudType[] = [];
        ...
        if (task.started_at && task.completed_at) {
            const vehicleRes = await client.query(
                'SELECT wash_time FROM vehicles WHERE type = $1',
                [task.car_type]
            );
        ...
        if (task.latitude && task.longitude) {
            const buildingRes = await client.query(
                `SELECT latitude, longitude, radius FROM buildings 
        ...
        for (const type of rules) {
            await client.query(
                `INSERT INTO fraud_cases (task_id, cleaner_id, type) 
                 VALUES ($1, $2, $3)
                 ON CONFLICT (task_id, type) DO NOTHING`,
                [task.id, task.cleaner_id, type]
            );
        }
    }
