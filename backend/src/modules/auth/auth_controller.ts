...
export const toggleUserStatusController = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
export const resetUserPasswordController = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    const data = await resetUserPasswordService(id as string, new_password);
