...
const createReview = async (req, res) => {
    try {
        ...
        const taskRes = await connectDatabase_1.pool.query('SELECT cleaner_id FROM tasks WHERE id = $1', [task_id]);
        if (!taskRes.rows.length) {
            return res.status(404).json({ success: false, message: 'Task not found' });
        }
        const cleaner_id = taskRes.rows[0].cleaner_id;
        // Check if review already exists for this task
        const existingReview = await connectDatabase_1.pool.query('SELECT id FROM reviews WHERE task_id = $1', [task_id]);
...
exports.getReviewsByWorker = getReviewsByWorker;
/* ================= GET REVIEW BY TASK ================= */
