...
const notification_service_2 = require("../notifications/notification_service");
...
const registerPushTokenController = async (req, res) => {
    try {
        ...
        const success = await (0, notification_service_2.registerPushToken)(userId.toString(), pushToken);
        if (success) {
            logger_1.logger.info(`Push token registered for user ${userId}`);
            return res.json({
                success: true,
        ...
        logger_1.logger.error('Register push token error', { err: error });
        return res.status(500).json({
            success: false,
            message: 'Failed to register push token',
        });
...
exports.getLiveWorkers = getLiveWorkers;
...
const assignTaskToWorker = async (req, res) => {
    try {
        ...
        const workerCheck = await connectDatabase_1.pool.query(`
      ...
      `, [worker_id, supervisorId]);
        ...
        try {
            await (0, notification_service_1.sendNotificationToUser)(worker_id, 'New Task Assigned', `${car_model} - ${car_number}`, {
                taskId: task.id,
...
exports.assignTaskToWorker = assignTaskToWorker;
/* ================= UPDATE TASK ================= */
...
const getAdminSupervisorDetails = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
        ...
        const supResult = await connectDatabase_1.pool.query(`SELECT 
        u.id AS user_id,
       ...
       WHERE (s.id = $1 OR u.id = $1) AND u.role = 'supervisor'`, [id]);
        if (!supResult.rows.length) {
            return res.status(404).json({ success: false, message: 'Supervisor not found' });
        }
        const sup = supResult.rows[0];
        // Get cleaners under this supervisor
        const cleanersResult = await connectDatabase_1.pool.query(`SELECT 
        c.id,
        u.full_name,
        u.email,
        c.total_tasks,
...
exports.getAdminSupervisorDetails = getAdminSupervisorDetails;
/* ================= ADMIN: UPDATE SUPERVISOR ================= */
const updateAdminSupervisor = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
            return res.status(400).json({ success: false, message: 'Supervisor ID is required' });
        ...
        const supCheck = await connectDatabase_1.pool.query(`SELECT u.id AS user_id, s.id AS supervisor_id, s.building_id 
       FROM users u 
       LEFT JOIN supervisors s ON u.id = s.user_id 
       WHERE (s.id = $1 OR u.id = $1) AND u.role = 'supervisor'`, [id]);
        ...
        if (userUpdates.length) {
            ...
            await connectDatabase_1.pool.query(`UPDATE users SET ${userUpdates.join(', ')}, updated_at = NOW() WHERE id = $${pi}`, userValues);
        }
        // Update or Insert into supervisors table
        if (building_id !== undefined || full_name !== undefined) {
            await connectDatabase_1.pool.query(`INSERT INTO supervisors (user_id, full_name, building_id)
         VALUES ($1, $2, $3)
         ON CONFLICT (user_id) DO UPDATE
           SET building_id = COALESCE(EXCLUDED.building_id, supervisors.building_id),
               ...
               updated_at = NOW()`, [userId, full_name || null, building_id !== undefined ? building_id : null]);
        }
        // Return the updated record
        const updated = await connectDatabase_1.pool.query(`SELECT 
        COALESCE(s.id, u.id) as id, 
        u.full_name, 
        u.email, 
        u.phone, 
...
exports.updateAdminSupervisor = updateAdminSupervisor;
/* ================= ADMIN: TOGGLE SUPERVISOR STATUS ================= */
const toggleAdminSupervisorStatus = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const result = await connectDatabase_1.pool.query(`UPDATE supervisors SET is_active = $1 WHERE id = $2 RETURNING id, is_active`, [is_active, id]);
        if (!result.rows.length) {
            return res.status(404).json({ success: false, message: 'Supervisor not found' });
        }
...
exports.toggleAdminSupervisorStatus = toggleAdminSupervisorStatus;
/* ================= ADMIN: DELETE SUPERVISOR ================= */
const deleteAdminSupervisor = async (req, res) => {
    try {
        const { id } = req.params;
        // Block deletion if there are assigned cleaners
        const cleanerCheck = await connectDatabase_1.pool.query('SELECT COUNT(*) FROM cleaners WHERE supervisor_id = $1', [id]);
        ...
        const supCheck = await connectDatabase_1.pool.query('SELECT user_id FROM supervisors WHERE id = $1', [id]);
        if (!supCheck.rows.length) {
            return res.status(404).json({ success: false, message: 'Supervisor not found' });
        }
        const userId = supCheck.rows[0].user_id;
        await connectDatabase_1.pool.query('DELETE FROM supervisors WHERE id = $1', [id]);
        await connectDatabase_1.pool.query('DELETE FROM users WHERE id = $1', [userId]);
